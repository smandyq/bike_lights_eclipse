...
{ 4, 16, 64, 255, 255, 232, 211, 192, 175, 159, 145, 132, 120, 110, 100, 91, 83,
		...
		2, 2, 1, 1, 1, 1, 1 };
star_t stars[MIDDLE_LED_COUNT];
star_t* stars_end = &stars[0] + MIDDLE_LED_COUNT;
const uint8_t* star_table_end = &star_table[0] + 64;

inline uint8_t get_next()
{
	if (!rando)
		rando = (MIDDLE_LED_COUNT - 1);
	return rando--;
}

void init_stars()
{
	for(uint8_t i=0; i<MIDDLE_LED_COUNT; i++)
	{
		stars[i].lut_index = (uint8_t*) star_table_end;
		stars[i].ticks = random(STAR_MAX_TICKS);
		stars[i].pixel = &middle_colors[i];
	}
